            ...
            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                @foreach($hairstyles as $hairstyle)
                    <tr class="hover:bg-gray-50">
                        <td class="pl-6 py-4">{{ $hairstyle->id }}</td>
                        <td class="px-6 py-4 font-medium text-gray-700">{{ $hairstyle->name }}</td>
                        <td class="px-6 py-4">
                            <img src="{{ Storage::url($hairstyle->image) }}" alt="{{ $hairstyle->name }}" 
                        ...
                        <td class="px-6 py-4 font-medium text-gray-700">{{ $hairstyle->category->name }}</td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <button wire:click="edit({{ $hairstyle->id }})" 
                                ...
                                <button wire:click="delete({{ $hairstyle->id }})"
                ...
                @endforeach
                <!-- Add this empty row that will always be present to maintain width -->
                @if(count($hairstyles) == 0)
                    <tr class="h-20">
                        <td colspan="6" class="text-center py-8 text-gray-500">No hairstyles found</td>
                    </tr>
                @endif
            </tbody>
        </table>
        ...
        <div class="fixed z-10 inset-0 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen">
                <div class="bg-white rounded-lg p-8 max-w-md w-full">
                    <h2 class="text-2xl font-bold mb-4">{{$isEditing ? 'Edit Hairstyle':'Add New Hairstyle'}}</h2>
                    ...
                    <form wire:submit.prevent="save">
                        ...
                        <div class="flex justify-end">
                            ...
                            <button type="submit" 
                                    class="bg-pink-500 text-white px-4 py-2 rounded-md">Save</button>
                        </div>
                    </form>
                </div>
            </div>
